    ...
    public Money total0() {
        ...
        for (Product p : carroActual) {
            if (p.getPrice().getCurrencyUnit() == CurrencyUnit.USD) {
                ...
                total += CurrencyConverter.convert(p.getPrice(),CurrencyUnit.USD, convertDollars).getAmount().doubleValue();
            }
        }
    ...
    public Money totalDto() {
        ...
        for (Product p : carroActual) {
            if (p.getPrice().getCurrencyUnit() == CurrencyUnit.USD) {
                ...
                total += p.applyDiscount(CurrencyConverter.convert(p.getPrice(),CurrencyUnit.USD,convertDollars) ,dtos.get(cont)).getAmount().doubleValue();
            }
            cont++;
        }
